...
export const enMessages: Messages = {
  ...
  basicParams: {
    ...
    accessoryCost: "BOS cost per module (excl. modules)",
    accessoryCostHint: (currency: string) =>
      `Total plant cost = modules + this line × module count. Units match settlement currency (${currency}/module).`,
    accessoryTotalPreview: (sym: string, total: string, count: string, perModule: number) =>
      `Approx. ${sym}${total} total BOS (${count} modules × ${perModule})`,
    epcHelperTitle: "EPC helper (optional)",
    targetEpc: (currency: string) => `Target EPC (${currency}/Wp)`,
    epcPlaceholder: "e.g. 3.5",
    applyAccessory: "Apply to per-module BOS cost",
    epcSuggestion: (power: number, sym: string, price: number, suggested: string) =>
      `Based on current LONGi module (${power} W, ${sym}${price}/W), suggested ${sym}${suggested}/module ((EPC − module price) × watts)`,
    operationYears: "Operating life (years)",
    currency: "Settlement currency",
    currencySearch: "Search currency…",
    currencySyncHint:
      "Synced with the header currency; PPA, BOS, and library prices convert on change.",
  ...
  report: {
    ...
    basicParamLabels: {
      ...
      ppa: "PPA price",
      accessory: "BOS cost (excl. modules)",
