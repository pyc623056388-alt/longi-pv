...
export const zhMessages = {
  ...
  basicParams: {
    ...
    accessoryCost: "除组件外全站成本 (每块组件)",
    accessoryCostHint: (currency: string) =>
      `全站投资 = 组件 + 本项 × 块数。单位与结算单位一致（${currency}/块）。`,
    accessoryTotalPreview: (sym: string, total: string, count: string, perModule: number) =>
      `当前约 ${sym}${total} 配件总成本（${count} 块 × ${perModule}）`,
    epcHelperTitle: "全站 EPC 辅助填数（可选）",
    targetEpc: (currency: string) => `目标全站 EPC (${currency}/Wp)`,
    epcPlaceholder: "例 3.5",
    applyAccessory: "应用到每块系统成本",
    epcSuggestion: (power: number, sym: string, price: number, suggested: string) =>
      `按当前隆基组件（${power}W、${sym}${price}/W）建议每块约 ${sym}${suggested}（(EPC − 组件单价) × 瓦数）`,
